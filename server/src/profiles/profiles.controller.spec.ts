...
describe('ProfilesController', () => {
  ...
  it('should call getMyProfile', async () => {
    ...
    mockProfilesService.getMyProfile.mockResolvedValue({
      id: 'p-1',
      userId: 'u-1',
    });
  ...
  it('should call updateMyProfile', async () => {
    ...
    mockProfilesService.upsertProfile.mockResolvedValue({
      id: 'p-1',
      fullName: 'Mahin',
    });
    ...
    expect(mockProfilesService.upsertProfile).toHaveBeenCalledWith(
      'u-1',
      'test@uni.edu',
      dto,
    );
  ...
  it('should call removeSkill', async () => {
    ...
    mockProfilesService.removeSkillFromProfile.mockResolvedValue({
      message: 'Skill removed',
    });

    const result = await controller.removeSkill(user, 's-1');
    expect(result).toEqual({ message: 'Skill removed' });
    expect(mockProfilesService.removeSkillFromProfile).toHaveBeenCalledWith(
      'u-1',
      's-1',
    );
  });
